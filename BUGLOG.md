...
## バグ #3 — iOS SafariでTTS（音声読み上げ）が動かない
- **日付**: 2025-06-30
- **症状**: iPad/iPhone Safariで問題文の読み上げが鳴らない
- **原因**: iOS SafariのSpeechSynthesis APIの既知バグ
  1. 初回呼び出し前に空のUtteranceで「キック」が必要
  2. 15秒後に勝手に停止する → resume()の定期呼び出しが必要
- **修正**: speak()前に空Utteranceを1回再生、speechSynthesis.resume()を5秒間隔で定期実行
- **教訓**: **iOS SafariはTTSにバグがある。最初からワークアラウンドを入れること。** デバッグで時間を使う前に、プラットフォーム別の既知問題を先に潰す。

---

## 教訓（変更前チェックリスト）
1. **変更前にBUGLOG.mdを確認** — 同じパターンのバグがないか
2. **依頼されていないコードに触れるな** — Q1表示変更のみ頼まれたのにマイクコードを勝手に書き換えたのが災祸の始まり
3. **1コミット1機能** — 複数変更を混ぜると原因切り分けが困難
4. **変更後は必ずブラウザで動作確認** — コード読みだけでは不十分
5. **録音機能に触れる変更は特に注意** — SpeechRecognitionとgetUserMediaは同時に使うな
6. **「直った」と言う前に配信コードをcurlで実証** — 推測で完了報告するな
7. **環境の問題とコードの問題を切り分けてから手を動かす** — 最小診断ページで先に切り分ける
8. **プラットフォーム別の既知バグを先に潰す** — iOS TTSバグは最初から対応すべきだった
